...
import { createToken, fetchToken } from './TokenAction';
...
export const createMarket = (history) => {
  return async function(dispatch) {
    ...
    marketRepository.create(INITIAL_AMOUNT, (transactionHash) => {
      ...
      dispatch(statusAddSuccessTransaction({
        address: market.marketContract.transactionHash,
        caption: "Creating contract",
        url: "/market"
      }));
      ...
      dispatch(hideModal());
      Cookies.set('market_address', market.marketContract.address);
      dispatch(redirectToMarket(history));
      dispatch(fetchToken(market.marketContract.address));
    }).catch((err) => {
...
export const getAllOffers = (address) => {
  return async function(dispatch) {
    ...
    const marketRepo = new Ambrosus.MarketRepository();
